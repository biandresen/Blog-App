...
const Terms = () => {
  return (
    <div className="container max-w-235">
      ...
      <section className="mt-4 rounded-xl border border-white/10 bg-[var(--primary)]/20 p-4">
        <p className="text-sm font-semibold text-[var(--text1)]">Related policies</p>
              ...
              <div className="mt-3 flex flex-wrap gap-3 text-sm">
                {termsContent.incorporatedPolicies.map((p) => (
                  <Link
                    key={p.path}
                    to={p.path}
                    className="rounded-full border border-white/10 bg-[var(--primary)] px-3 py-1 underline opacity-90 hover:opacity-100"
                  >
                    {p.label}
                  </Link>
                ))}
